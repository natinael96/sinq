...
Compact navigation uses equal-width bottom slots and persistent labels. At 600dp and above it becomes a Material navigation rail, while root content is bounded to a readable 840dp measure. Private surfaces use the complete-screen journal boundary. Content loading and retry share `ContentLoad`; completion is explicit and reversible. The dark liturgical red is `#FF8277`, measuring 6.08:1 on the background and 5.21:1 on the surface. Chapter/day controls expose selected or checked states; forms and long sheets scroll. These are source/build-verified changes, with device accessibility checks still outstanding. See [fix coverage](UI_UX_FIX_PROGRESS.md).
