...
# ማኅሌት movements that are chanted out of a book without naming it.
#
# The part-to-book link matches a movement's name against a book's title, which
# works while the ማኅሌት says መልክአ ሥላሴ and the shelf has a book so called. The
# ነግሥ movements say only ነግሥ. The rates below are how many of each movement's
# verses are actually findable in ዜማ ዘነግሥ, measured over the whole corpus —
# they are why the third name is not here:
#
#   ነግሥ            221 parts   78 land on a stanza   35%
#   ሥርዓተ ነግሥ       153 parts   16 land on a stanza   10%
#   እግዚአብሔር ነግሠ     83 parts    0 land on a stanza    0%
#
# A movement that never matches is a different chant wearing a similar name,
# and aliasing it would open a book that does not contain what is being sung.
ALT_KEYS = {
    "ዜማ ዘነግሥ": ["ነግሥ", "ሥርዓተ ነግሥ"],
}
...
# Red, as the printed page has it.
#
# A መልክእ's red can be derived — the salutation opens every stanza and the rule
# is in the app. A ዜማ book's cannot: the printer reddens the incipit, the
# performance directions and a name when it carries the stanza, and leaves the
# same name black two lines later. That is an editorial decision per line, so
# it is carried in the source rather than guessed at read time.
#
# The marker travels through clean_text — which rewrites whitespace and would
# invalidate any offsets written by hand — and is turned into offsets here,
# after the text is final.
RED_OPEN, RED_CLOSE = "\u00ab", "\u00bb"


def extract_red(text):
    """(text without markers, [[start, end], ...] into that text)."""
    if RED_OPEN not in text:
        return text, []
    out, spans, start = [], [], None
    length = 0
    for ch in text:
        if ch == RED_OPEN:
            start = length
...
def build():
    ...
    for path in sorted(p for d in SOURCES for p in d.glob("*.json")):
        ...
        if "chapters" not in data:
            if "sections" in data:
                # Structured Melkea: synthesize chapter blocks for the app reader
                m_title = (data.get("title", {}).get("gez") if isinstance(data.get("title"), dict) else data.get("title")) or raw_title
                blocks = [{"type": "heading", "level": 1, "text": m_title}]
                for sec in data["sections"]:
                    sec_heading = sec.get("title") or sec.get("heading")
                    if sec_heading and sec_heading != m_title and sec_heading != "ስንኞች (አካላት)":
                        blocks.append({"type": "heading", "level": 2, "text": sec_heading})
        ...
        books.append({
            ...
            "meta": {
                ...
                "key": fold(title),
                **(
                    {"altKeys": [fold(a) for a in ALT_KEYS[title]]}
                    if title in ALT_KEYS else {}
                ),
                ...
                **({"comingSoon": True} if title in COMING_SOON else {}),
            },
        })
