...
def check_synaxarium() -> None:
    ...
    """
    manifest_path = os.path.join(CONTENT, "sinksar", "manifest.json")
    if not os.path.isfile(manifest_path):
        print("sinksar: content not bundled (coming soon)")
        return
...
def check_books() -> None:
    ...
    available_listed = {bid: meta for bid, meta in listed.items() if not meta.get("comingSoon")}
    for orphan in sorted(on_disk - set(listed)):
        fail(f"books/{orphan}.json is not listed in the index")
    for missing in sorted(set(available_listed) - on_disk):
    ...
    for bid, meta in available_listed.items():
    ...
    for bid in sorted(available_listed):
        raw = os.path.join(CONTENT, "books", f"{bid}.json")
        with open(raw, encoding="utf-8") as f:
            if "፪ማ፡" in f.read():
                fail(f"books/{bid}.json still carries a ፪ማ፡ tier mark")

    print(f"books: {len(listed)} books ({len(available_listed)} available), {chapters} chapters, {blocks} blocks, {chars} chars")
