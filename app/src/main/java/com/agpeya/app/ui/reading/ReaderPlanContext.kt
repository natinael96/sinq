...
internal fun readerPlanDays(
    ...
    // dueDay, not dayOn: a day that was never finished is still the day to
    // read, so missing day 2 does not hand the reader day 3 with day 2 left
    // behind. An explicit assignment from a link still wins over both.
    val number = if (selectedPlanId.isNotBlank()) selectedPlanDay
        else ReadingPlanRepository.dueDay(
            state, plan.id, days, kept.startedOn, today, plan.days,
        )
