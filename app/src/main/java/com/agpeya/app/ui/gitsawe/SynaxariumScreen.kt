...
import com.agpeya.app.ui.common.appendRubricated
...
private fun ClosingPrayer(fontSp: Int) {
    ...
    Column(
        ...
        SYNAXARIUM_CLOSING_STANZAS.forEach { stanza ->
            ...
            Text(
                // SINKSAR: these are salam verses, so the opening
                // salutation reddens along with the names — the same rule the
    ...
    Box(Modifier.fillMaxWidth().padding(bottom = Spacing.md)) {
        androidx.compose.foundation.text.selection.SelectionContainer {
            Column(
                ...
                Text(
                    // GENERAL, not SINKSAR: this block is a Gospel or epistle
                    // passage, and reddening every ጴጥሮስ in it would mark the
