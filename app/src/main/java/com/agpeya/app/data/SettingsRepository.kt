...
/**
 * How a fired reminder alerts: ring + vibrate, ring only, vibrate only, or
 * silent.
 *
 * [VIBRATE_ONLY] is the default. An hour arrives in church as often as it
 * arrives at home, and an alarm that rings out during ቅዳሴ is a worse failure
 * than one that is missed: the reader can always turn sound on, but nobody gets
 * to take back the noise. Vibration still summons, and summons only its owner.
 */
...
object SettingsRepository {
    ...
    data class BackupSettings(
        ...
        val alarmAlert: String = AlarmAlert.VIBRATE_ONLY.name,
    ...
    private val KEY_SHOW_ALARM_SOUND_NOTICE = booleanPreferencesKey("show_alarm_sound_notice")
    ...
    suspend fun backupSettings(context: Context): BackupSettings {
        ...
        return BackupSettings(
            ...
            alarmAlert = prefs[KEY_ALARM_ALERT] ?: AlarmAlert.VIBRATE_ONLY.name,
    ...
    suspend fun restoreSettings(context: Context, value: BackupSettings) {
        context.settingsDataStore.edit { prefs ->
            ...
            prefs[KEY_ALARM_ALERT] = runCatching { AlarmAlert.valueOf(value.alarmAlert) }.getOrDefault(AlarmAlert.VIBRATE_ONLY).name
    ...
    /** The reading plan's nudge. On, but silent until a plan is actually started. */
    fun readingReminder(context: Context): Flow<Boolean> =
    ...
    fun alarmAlert(context: Context): Flow<AlarmAlert> =
        context.settingsDataStore.data.map {
            runCatching { AlarmAlert.valueOf(it[KEY_ALARM_ALERT] ?: "") }.getOrDefault(AlarmAlert.VIBRATE_ONLY)
        }
    ...
    fun alarmAlertBlocking(context: Context): AlarmAlert =
        ...
        }.getOrDefault(AlarmAlert.VIBRATE_ONLY)
    ...
    /**
     * Whether the reader still has to be told that an arriving hour vibrates
     * rather than rings — see [AlarmAlert].
     *
     * Raised once the notification permission is in place and not before: until
     * then there is nothing that can ring, and a notice about the manner of an
     * alarm that cannot fire is noise. Shown once, and never to someone who has
     * already chosen for themselves.
     *
     * **Absent means "not yet told", not "nothing to tell"** — hence `?: true`
     * rather than arming this at onboarding. The readers who most need the
     * notice are the ones already installed when the default changed: their
     * alarms rang until they updated and will vibrate afterwards, and they will
     * never onboard again. Anyone who had picked an alert of their own is spent
     * silently by the caller.
     */
    fun showAlarmSoundNotice(context: Context): Flow<Boolean> =
        context.settingsDataStore.data.map { it[KEY_SHOW_ALARM_SOUND_NOTICE] ?: true }

    suspend fun dismissAlarmSoundNotice(context: Context) {
        context.settingsDataStore.edit { it[KEY_SHOW_ALARM_SOUND_NOTICE] = false }
    }
}
