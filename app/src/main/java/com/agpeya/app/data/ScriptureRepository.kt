    ...
    suspend fun psalms(context: Context, geez: Boolean = false): List<com.agpeya.app.model.Section> =
        psalmCache[geez] ?: withContext(Dispatchers.IO) {
            ...
            runCatching {
                ...
                // runCatching catches Throwable, CancellationException with it,
                // so a switch of edition that cancels a parse half way used to
                // be reported as a failed load and logged as one. Cancellation
                // is not a failure: it means the answer is no longer wanted.
                .onFailure {
