...
object BookRepository {
     ...
     *
     * The title is tried first and the aliases after, so a book can never lose
     * its own name to another book's alias.
     */
    suspend fun byPartName(context: Context, partName: String): BookMeta? {
        val key = fold(partName)
        if (key.isBlank()) return null
        val books = all(context)
        return books.find { it.key == key } ?: books.find { key in it.altKeys }
    }
