...
class ReadingReminderReceiver : BroadcastReceiver() {
        ...
        Thread {
            try {
                runBlocking {
                    ...
                    // A plan waiting on a day that was missed is not "today's
                    // reading", and a notice that says it is teaches the reader
                    // to distrust the number underneath it. When the plan is
                    // behind the nudge says so and offers to take them back to
                    // where they stopped — which is where tapping it lands,
                    // because that is the day the reader is handed.
                    val behind = ReadingPlanRepository.isBehind(content, state, today)
                    val body = body(context, unread, s, behind)
                    ReminderDispatchGate.locked {
                        if (SettingsRepository.readingReminderBlocking(context) &&
                            ...
                            notify(
                                context,
                                if (behind) s.readingResumeTitle else s.readingReminderTitle,
                                body,
                                s,
                            )
                        }
    ...
    private suspend fun body(
        ...
        return when {
            passages.isBlank() && behind -> s.readingResumePlain(firstDay)
            passages.isBlank() -> s.readingReminderPlain(firstDay)
            behind -> s.readingResumeBody(firstDay, passages)
