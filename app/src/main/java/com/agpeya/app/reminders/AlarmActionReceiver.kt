...
class AlarmActionReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val end = AlarmEndSessions.claim(session)
        if (end == AlarmEndSessions.End.ALREADY) return
        // A superseded ring may still be followed up — the prompt is per hour,
        // under its own notification id — but it must not take down the
        // notification, which now belongs to the ring that replaced it.
        val onScreen = end == AlarmEndSessions.End.CURRENT
        when (action) {
            AlarmRinger.ACTION_SNOOZE -> {
                ...
                AlarmRinger.cancelTimeout(context, session)
                if (onScreen) AlarmRinger.stop(context)
